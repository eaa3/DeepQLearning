...
An WaterWorld environemtn to play
]]
...
function WaterWorld:__init(width,height)
    ...
    self.n_eaten_food = 1.0
    self.n_eaten_poison = 1.0

    for i = 1, self.max_elems do
        local food = Food(false,self.world,love.physics.getMeter(),2)
        local poison = Food(true,self.world,love.physics.getMeter(),2)
...
function WaterWorld:getStateDim()
    return self.state_dim
end

function WaterWorld:getNActions()
    ...
    for i = 1, self.max_elems do
        ...
        if food.is_active then
            ...
            if d < radius and food.is_poison then
                ...
                self.n_eaten_poison = self.n_eaten_poison + 1
                --print("colided with poison!")
            elseif d < radius and not food.is_poison then
                reward = reward + 1
                food.is_active = false
                table.insert(self.inactive_list, food)
                self.n_eaten_food = self.n_eaten_food + 1
...
function WaterWorld:step(action, dt)
    ...
    -- Compute reward
    local reward = self:computeRewardAndUpdate()
